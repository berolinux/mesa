                                            ...
                                            int *host_sema_mode_out)
{
   ...
   for (i = 0; i < n; i++) {
       ...
       * formal sema; modes from pass21 ladder with sticky first) */
      {
         ...
         /* tick190 / pass27: pass26 timed out — pass27 G1 entry */
         if (NV_PASS27_RE_SCAFFOLD && NV_PASS27_G1_CE_PASS26 &&
         ...
         /* tick194 / pass29: pass28 timed out — pass29 G1 entry */
         if (NV_PASS29_RE_SCAFFOLD && NV_PASS29_G1_CE_PASS28 &&
         ...
         /* tick196 / pass30: pass29 timed out — pass30 G1 entry */
         if (NV_PASS30_RE_SCAFFOLD && NV_PASS30_G1_CE_PASS29 &&
         ...
         /* tick198 / pass31: pass30 timed out — pass31 G1 entry */
         if (NV_PASS31_RE_SCAFFOLD && NV_PASS31_G1_CE_PASS30 &&
            ...
            if (nv_pass22_compute_object_build(&p22obj) == 0) {
               ...
               if (emit_ok != 0 && NV_PASS30_RE_SCAFFOLD && nv_pass30_policy_ok() &&
                  ...
                  emit_ok = nv_pass30_compute_object_emit_launch(
                     &push, cc_try, &p22obj, lmem_gpu_addr, true,
                     sema_gpu_addr, sema_payload, hs_mode);
               if (emit_ok != 0 && NV_PASS29_RE_SCAFFOLD && nv_pass29_policy_ok() &&
                   nv_pass29_g0_g4_symmetry_ok())
                  emit_ok = nv_pass29_compute_object_emit_launch(
                     &push, cc_try, &p22obj, lmem_gpu_addr, true,
                     sema_gpu_addr, sema_payload, hs_mode);
               if (emit_ok != 0 && NV_PASS28_RE_SCAFFOLD && nv_pass28_policy_ok() &&
                   nv_pass28_g0_g4_symmetry_ok())
                  emit_ok = nv_pass28_compute_object_emit_launch(
                     &push, cc_try, &p22obj, lmem_gpu_addr, true,
                     sema_gpu_addr, sema_payload, hs_mode);
               if (emit_ok != 0 && NV_PASS27_RE_SCAFFOLD && nv_pass27_policy_ok() &&
                   nv_pass27_g0_g4_symmetry_ok())
                  emit_ok = nv_pass27_compute_object_emit_launch(
                     &push, cc_try, &p22obj, lmem_gpu_addr, true,
                     sema_gpu_addr, sema_payload, hs_mode);
               if (emit_ok != 0 && NV_PASS26_RE_SCAFFOLD && nv_pass26_policy_ok() &&
                   nv_pass26_g0_g4_symmetry_ok())
                                   ...
                                   uint32_t *class_used_out)
{
   ...
   for (i = 0; i < n; i++) {
      ...
      if (sema_gpu_addr && sema_cpu) {
         ...
         /* tick190 / pass27: pass26 timed out — pass27 G3 inv+host */
         if (NV_PASS27_RE_SCAFFOLD && NV_PASS27_G3_3D_PASS26_BARRIER &&
             nv_pass27_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need);
         ...
         /* tick194 / pass29: pass28 timed out — pass29 G3 inv+host */
         if (NV_PASS29_RE_SCAFFOLD && NV_PASS29_G3_3D_PASS28_BARRIER &&
             nv_pass29_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need);
         ...
         /* tick196 / pass30: pass29 timed out — pass30 G3 inv+host */
         if (NV_PASS30_RE_SCAFFOLD && NV_PASS30_G3_3D_PASS29_BARRIER &&
             nv_pass30_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need);
         ...
         /* tick198 / pass31: pass30 timed out — pass31 G3 inv+host */
         if (NV_PASS31_RE_SCAFFOLD && NV_PASS31_G3_3D_PASS30_BARRIER &&
             nv_pass31_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need);
            ...
            if (c3)
               nv_3d_set_object(&push, c3);
            if (nv_3d_emit_g3_inv_wfi_host_sema_pass31(
                   &push, sema_gpu_addr, sema_payload, hs_mode, true) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
               r = nv_channel_submit_wait_sema(ch, sema_cpu, sema_payload,
                                               wait_timeout_ns, check_notifier);
                                         ...
                                         uint32_t *class_used_out)
{
   ...
   for (i = 0; i < n && i < 12; i++) {
      ...
      /* tick170 / pass23: pass17 host tail timed out — pass21 then pass23 (NVDEC parity) */
      {
         ...
         /* tick190 / pass27: pass24 timed out — pass27 NVENC bringup */
         if (NV_PASS27_RE_SCAFFOLD && NV_PASS27_G4_VIDEO_PASS26_PASS27 &&
             nv_pass27_g0_g4_symmetry_ok()) {
            if (sema_reset)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need + 40);
         ...
         /* tick194 / pass29: pass28 timed out — pass29 NVENC bringup */
         if (NV_PASS29_RE_SCAFFOLD && NV_PASS29_G4_VIDEO_PASS28_PASS29 &&
             nv_pass29_g0_g4_symmetry_ok()) {
            if (sema_reset)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need + 40);
            if (!map)
               return -ENOMEM;
            nv_push_init(&push, map, need + 40);
            if (nv_g4_emit_nvenc_bringup_pass29(
                   &push, ce, pic_setup_gpu, in_buf_gpu, bs_buf_gpu, status_gpu,
                   width, height, sema_gpu_addr, sema_payload, NULL, true,
                   hs) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
         ...
         /* tick196 / pass30: pass29 timed out — pass30 NVENC bringup */
         if (NV_PASS30_RE_SCAFFOLD && NV_PASS30_G4_VIDEO_PASS29_PASS30 &&
             nv_pass30_g0_g4_symmetry_ok()) {
            if (sema_reset)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need + 40);
            if (!map)
               return -ENOMEM;
            nv_push_init(&push, map, need + 40);
            if (nv_g4_emit_nvenc_bringup_pass30(
                   &push, ce, pic_setup_gpu, in_buf_gpu, bs_buf_gpu, status_gpu,
                   width, height, sema_gpu_addr, sema_payload, NULL, true,
                   hs) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
         ...
         /* tick198 / pass31: pass30 timed out — pass31 NVENC bringup */
         if (NV_PASS31_RE_SCAFFOLD && NV_PASS31_G4_VIDEO_PASS30_PASS31 &&
             nv_pass31_g0_g4_symmetry_ok()) {
            if (sema_reset)
               sema_cpu[0] = 0;
            map = nv_channel_push_begin(ch, need + 40);
            if (!map)
               return -ENOMEM;
            nv_push_init(&push, map, need + 40);
            if (nv_g4_emit_nvenc_bringup_pass31(
                   &push, ce, pic_setup_gpu, in_buf_gpu, bs_buf_gpu, status_gpu,
                   width, height, sema_gpu_addr, sema_payload, NULL, true,
                   hs) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
                                    ...
                                    uint32_t *class_used_out)
{
   ...
   for (i = 0; i < n; i++) {
      ...
      /* tick169 / pass23: pass17 host tail timed out — pass21 then pass23 bringup */
      {
         ...
         /* tick190 / pass27: pass26 timed out — pass27 NVDEC bringup */
         if (NV_PASS27_RE_SCAFFOLD && NV_PASS27_G4_VIDEO_PASS26_PASS27 &&
             nv_pass27_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
         ...
         /* tick194 / pass29: pass28 timed out — pass29 NVDEC bringup */
         if (NV_PASS29_RE_SCAFFOLD && NV_PASS29_G4_VIDEO_PASS28_PASS29 &&
             nv_pass29_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            ...
            if (!map)
               return -ENOMEM;
            nv_push_init(&push, map, need + 40);
            if (nv_g4_emit_nvdec_bringup_pass29(&push, cl, pic, sema_gpu_addr,
                                                sema_payload, sema_cpu, true,
                                                hs) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
               r = nv_channel_submit_wait_sema(ch, sema_cpu, sema_payload,
         ...
         /* tick196 / pass30: pass29 timed out — pass30 NVDEC bringup */
         if (NV_PASS30_RE_SCAFFOLD && NV_PASS30_G4_VIDEO_PASS29_PASS30 &&
             nv_pass30_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            ...
            if (!map)
               return -ENOMEM;
            nv_push_init(&push, map, need + 40);
            if (nv_g4_emit_nvdec_bringup_pass30(&push, cl, pic, sema_gpu_addr,
                                                sema_payload, sema_cpu, true,
                                                hs) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
               r = nv_channel_submit_wait_sema(ch, sema_cpu, sema_payload,
         ...
         /* tick198 / pass31: pass30 timed out — pass31 NVDEC bringup */
         if (NV_PASS31_RE_SCAFFOLD && NV_PASS31_G4_VIDEO_PASS30_PASS31 &&
             nv_pass31_g0_g4_symmetry_ok()) {
            if (sema_reset && sema_cpu)
               sema_cpu[0] = 0;
            ...
            if (!map)
               return -ENOMEM;
            nv_push_init(&push, map, need + 40);
            if (nv_g4_emit_nvdec_bringup_pass31(&push, cl, pic, sema_gpu_addr,
                                                sema_payload, sema_cpu, true,
                                                hs) == 0) {
               nv_channel_push_advance(ch, nv_push_dw_count(&push));
               r = nv_channel_submit_wait_sema(ch, sema_cpu, sema_payload,
