...
/**
 * tick178 / pass24: G4 NVDEC/NVENC bringup with pass24 policy gate; same as pass23.
 */
static inline int
nv_g4_emit_nvdec_bringup_pass24(struct nv_push *p, uint32_t class_nvdec,
                                const struct nv_nvdec_pic_setup *pic,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                volatile uint32_t *status_cpu,
                                bool add_host_sema_tail,
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass24_explicit_emit_required() && NV_PASS24_RE_INHERITS_PASS23)
      return -1;
   return nv_g4_emit_nvdec_bringup_pass23(p, class_nvdec, pic, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
}

static inline int
nv_g4_emit_nvenc_bringup_pass24(struct nv_push *p, uint32_t class_nvenc,
                                uint64_t pic_setup_gpu, uint64_t input_yuv_gpu,
                                uint64_t bitstream_gpu, uint64_t status_gpu,
                                uint32_t width, uint32_t height,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                ...
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass24_explicit_emit_required() && NV_PASS24_RE_INHERITS_PASS23)
      return -1;
   return nv_g4_emit_nvenc_bringup_pass23(p, class_nvenc, pic_setup_gpu,
                                          input_yuv_gpu, bitstream_gpu,
                                          status_gpu, width, height, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
...
/**
 * tick182 / pass25: G4 NVDEC/NVENC bringup with pass25 policy gate.
 */
static inline int
nv_g4_emit_nvdec_bringup_pass25(struct nv_push *p, uint32_t class_nvdec,
                                const struct nv_nvdec_pic_setup *pic,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                volatile uint32_t *status_cpu,
                                bool add_host_sema_tail,
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass25_policy_ok() && NV_PASS25_RE_INHERITS_PASS24)
      return -1;
   return nv_g4_emit_nvdec_bringup_pass24(p, class_nvdec, pic, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
}

static inline int
nv_g4_emit_nvenc_bringup_pass25(struct nv_push *p, uint32_t class_nvenc,
                                uint64_t pic_setup_gpu, uint64_t input_yuv_gpu,
                                uint64_t bitstream_gpu, uint64_t status_gpu,
                                uint32_t width, uint32_t height,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                ...
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass25_policy_ok() && NV_PASS25_RE_INHERITS_PASS24)
      return -1;
   return nv_g4_emit_nvenc_bringup_pass24(p, class_nvenc, pic_setup_gpu,
                                          input_yuv_gpu, bitstream_gpu,
                                          status_gpu, width, height, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
...
/**
 * tick186 / pass26: G4 NVDEC/NVENC bringup with pass26 policy gate.
 */
static inline int
nv_g4_emit_nvdec_bringup_pass26(struct nv_push *p, uint32_t class_nvdec,
                                const struct nv_nvdec_pic_setup *pic,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                volatile uint32_t *status_cpu,
                                bool add_host_sema_tail,
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass26_policy_ok() && NV_PASS26_RE_INHERITS_PASS25)
      return -1;
   return nv_g4_emit_nvdec_bringup_pass25(p, class_nvdec, pic, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
}

static inline int
nv_g4_emit_nvenc_bringup_pass26(struct nv_push *p, uint32_t class_nvenc,
                                uint64_t pic_setup_gpu, uint64_t input_yuv_gpu,
                                uint64_t bitstream_gpu, uint64_t status_gpu,
                                uint32_t width, uint32_t height,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                ...
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass26_policy_ok() && NV_PASS26_RE_INHERITS_PASS25)
      return -1;
   return nv_g4_emit_nvenc_bringup_pass25(p, class_nvenc, pic_setup_gpu,
                                          input_yuv_gpu, bitstream_gpu,
                                          status_gpu, width, height, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
...
/**
 * tick190 / pass27: G4 NVDEC bringup with pass27 host sema tail.
 */
static inline int
nv_g4_emit_nvdec_bringup_pass27(struct nv_push *p, uint32_t class_nvdec,
                                const struct nv_nvdec_pic_setup *pic,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                volatile uint32_t *status_cpu,
                                bool add_host_sema_tail,
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass27_policy_ok() && NV_PASS27_RE_INHERITS_PASS26)
      return -1;
   return nv_g4_emit_nvdec_bringup_pass26(p, class_nvdec, pic, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
}

static inline int
nv_g4_emit_nvenc_bringup_pass27(struct nv_push *p, uint32_t class_nvenc,
                                uint64_t pic_setup_gpu, uint64_t input_yuv_gpu,
                                uint64_t bitstream_gpu, uint64_t status_gpu,
                                uint32_t width, uint32_t height,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                ...
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass27_policy_ok() && NV_PASS27_RE_INHERITS_PASS26)
      return -1;
   return nv_g4_emit_nvenc_bringup_pass26(p, class_nvenc, pic_setup_gpu,
                                          input_yuv_gpu, bitstream_gpu,
                                          status_gpu, width, height, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
...
/**
 * tick192 / pass28: G4 NVDEC bringup with pass28 host sema tail.
 */
static inline int
nv_g4_emit_nvdec_bringup_pass28(struct nv_push *p, uint32_t class_nvdec,
                                const struct nv_nvdec_pic_setup *pic,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                volatile uint32_t *status_cpu,
                                bool add_host_sema_tail,
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass28_policy_ok() && NV_PASS28_RE_INHERITS_PASS27)
      return -1;
   return nv_g4_emit_nvdec_bringup_pass27(p, class_nvdec, pic, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
}

static inline int
nv_g4_emit_nvenc_bringup_pass28(struct nv_push *p, uint32_t class_nvenc,
                                uint64_t pic_setup_gpu, uint64_t input_yuv_gpu,
                                uint64_t bitstream_gpu, uint64_t status_gpu,
                                uint32_t width, uint32_t height,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                ...
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass28_policy_ok() && NV_PASS28_RE_INHERITS_PASS27)
      return -1;
   return nv_g4_emit_nvenc_bringup_pass27(p, class_nvenc, pic_setup_gpu,
                                          input_yuv_gpu, bitstream_gpu,
                                          status_gpu, width, height, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
...
/**
 * tick196 / pass30: G4 NVDEC bringup with pass30 host sema tail.
 */
static inline int
nv_g4_emit_nvdec_bringup_pass30(struct nv_push *p, uint32_t class_nvdec,
                                const struct nv_nvdec_pic_setup *pic,
                                uint64_t sema_gpu, uint32_t sema_payload,
                                volatile uint32_t *status_cpu,
                                bool add_host_sema_tail,
                                enum nv_host_sema_mode host_sema_mode)
{
   if (!nv_pass30_policy_ok() && NV_PASS30_RE_INHERITS_PASS29)
      return -1;
   return nv_g4_emit_nvdec_bringup_pass29(p, class_nvdec, pic, sema_gpu,
                                          sema_payload, status_cpu,
                                          add_host_sema_tail, host_sema_mode);
}

static inline int
nv_g4_emit_nvenc_bringup_pass30(struct nv_push *p, uint32_t class_nvenc,
                                uint64_t pic_setup_gpu, uint64_t input_yuv_gpu,
                                uint64_t bitstream_gpu, uint64_t status_gpu,
                                uint32_t width, uint32_t height,
                                uint64_t sema_gpu, uint32_t sema_payload,
