...
/* --- MME (method macro engine) upload scaffolding (clc597.h) ---
 * Proprietary indirect multi-draw loops are MME programs.  Until we ship a
 * full indirect-draw MME binary, expose load/start helpers so callers can
 * upload firmware-derived microcode later and invoke by start index.
 */
...
#define NVC597_SET_MME_SHADOW_RAM_CONTROL_MODE_METHOD_TRACK  0x0
#define NVC597_SET_MME_SHADOW_RAM_CONTROL_MODE_METHOD_PASSTHROUGH 0x2
#define NVC597_SET_MME_VERSION                    0x0234
#define NVC597_SET_MME_MEM_ADDRESS_A              0x0550
#define NVC597_SET_MME_MEM_ADDRESS_B              0x0554
#define NVC597_SET_MME_DATA                       0x0558
#define NVC597_SET_MME_DATA_FIFO_CONFIG           0x055c
/* CALL_MME_MACRO(i) method base — macro index i in [0..127] at 0x3800 + i*8? */
#define NVC597_CALL_MME_MACRO(i)                  (0x3800 + ((i) & 0x7fu) * 8)
#define NVC597_CALL_MME_DATA(i)                   (0x3804 + ((i) & 0x7fu) * 8)

/** Upload MME instruction words starting at ram_pointer (dword index). */
static inline void
nv_3d_mme_load_instructions(struct nv_push *p, uint32_t ram_pointer,
                            const uint32_t *insns, uint32_t insn_count)
{
   uint32_t i;
   if (!p || !insns || !insn_count)
      return;
   nv_push_method(p, NVC597_LOAD_MME_INSTRUCTION_RAM_POINTER, ram_pointer);
   for (i = 0; i < insn_count; i++)
      nv_push_method(p, NVC597_LOAD_MME_INSTRUCTION_RAM, insns[i]);
}

/** Program MME start address slot for macro index. */
static inline void
nv_3d_mme_set_start_address(struct nv_push *p, uint32_t start_slot,
                            uint32_t instr_ram_offset)
{
   if (!p)
      return;
   nv_push_method(p, NVC597_LOAD_MME_START_ADDRESS_RAM_POINTER, start_slot);
   nv_push_method(p, NVC597_LOAD_MME_START_ADDRESS_RAM, instr_ram_offset);
}

/** Invoke MME macro i with optional first data dword (method pair). */
static inline void
nv_3d_mme_call_macro(struct nv_push *p, uint32_t macro_index, uint32_t data0)
{
   if (!p)
      return;
 ...
 */
static inline void
nv_3d_mme_set_shadow_ram_control(struct nv_push *p, uint32_t mode)
{
   if (!p)
      return;
...
/**
 * tick178 / pass24: G3 full barrier + pass24 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass24(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
...
/**
 * tick178 / pass24: G3 inv ladder + WFI + pass24 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass24(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
...
/**
 * tick182 / pass25: G3 full barrier + pass25 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass25(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
...
/**
 * tick182 / pass25: G3 inv ladder + WFI + pass25 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass25(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
...
/**
 * tick186 / pass26: G3 full barrier + pass26 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass26(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
...
/**
 * tick186 / pass26: G3 inv ladder + WFI + pass26 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass26(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
...
/**
 * tick190 / pass27: G3 full barrier + pass27 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass27(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
   ...
   if (!host_sema_gpu)
      return 0;
   return nv_push_g0_g4_host_sema_tail_pass27(
      p, false, host_sema_gpu,
      host_sema_payload ? host_sema_payload : 1u, host_sema_mode);
}

/**
 * tick190 / pass27: G3 inv ladder + WFI + pass27 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass27(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
...
/**
 * tick192 / pass28: G3 full barrier + pass28 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass28(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
   ...
   if (!host_sema_gpu)
      return 0;
   return nv_push_g0_g4_host_sema_tail_pass28(
      p, false, host_sema_gpu,
      host_sema_payload ? host_sema_payload : 1u, host_sema_mode);
}

/**
 * tick192 / pass28: G3 inv ladder + WFI + pass28 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass28(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
...
/**
 * tick194 / pass29: G3 full barrier + pass29 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass29(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
   ...
   if (!host_sema_gpu)
      return 0;
   return nv_push_g0_g4_host_sema_tail_pass29(
      p, false, host_sema_gpu,
      host_sema_payload ? host_sema_payload : 1u, host_sema_mode);
}

/**
 * tick194 / pass29: G3 inv ladder + WFI + pass29 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass29(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
...
/**
 * tick196 / pass30: G3 full barrier + pass30 host sema tail.
 */
static inline int
nv_3d_emit_g3_barrier_all_pass30(struct nv_push *p,
                                 uint64_t host_sema_gpu,
                                 uint32_t host_sema_payload,
                                 enum nv_host_sema_mode host_sema_mode)
{
...
/**
 * tick196 / pass30: G3 inv ladder + WFI + pass30 host sema.
 */
static inline int
nv_3d_emit_g3_inv_wfi_host_sema_pass30(struct nv_push *p,
                                       uint64_t sema_gpu_addr,
                                       uint32_t sema_payload,
                                       enum nv_host_sema_mode sema_mode,
                                       bool pre_wfi_on_3d)
